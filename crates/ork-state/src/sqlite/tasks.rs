...
impl SqliteDatabase {
    ...
    pub(super) async fn list_tasks_impl(&self, run_id: Uuid) -> Result<Vec<Task>> {
        let rows = sqlx::query_as::<_, TaskRow>("SELECT * FROM tasks WHERE run_id = ? ORDER BY task_index")
    ...
    pub(super) async fn get_running_tasks_impl(&self) -> Result<Vec<Task>> {
        let rows = sqlx::query_as::<_, TaskRow>("SELECT * FROM tasks WHERE status IN ('running', 'dispatched')")
            .fetch_all(&self.pool).await?;
        Ok(rows.into_iter().map(Self::map_task).collect())
    }

    pub(super) async fn append_task_log_impl(&self, task_id: Uuid, chunk: &str) -> Result<()> {
        sqlx::query("UPDATE tasks SET logs = COALESCE(SUBSTR(logs, 1, 2000000), '') || ? WHERE id = ?")
            .bind(chunk).bind(task_id).execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub(super) async fn update_task_output_impl(&self, task_id: Uuid, output: serde_json::Value) -> Result<()> {
        sqlx::query("UPDATE tasks SET output = ? WHERE id = ?")
            .bind(sqlx::types::Json(output)).bind(task_id).execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub(super) async fn get_task_outputs_impl(&self, run_id: Uuid, task_names: &[String]) -> Result<HashMap<String, serde_json::Value>> {
        ...
        let query_str = format!("SELECT task_name, output FROM tasks WHERE run_id = ? AND task_name IN ({}) AND output IS NOT NULL", placeholders);
    ...
    pub(super) async fn get_task_retry_meta_impl(&self, task_ids: &[Uuid]) -> Result<HashMap<Uuid, (i32, i32)>> {
        ...
        let query_str = format!("SELECT id, attempts, max_retries FROM tasks WHERE id IN ({})", placeholders);
