...
impl PostgresDatabase {
    ...
    pub(super) async fn get_run_impl(&self, run_id: Uuid) -> Result<Run> {
        let run = sqlx::query_as::<_, Run>("SELECT * FROM runs WHERE id = $1").bind(run_id).fetch_one(&self.pool).await?;
        Ok(run)
    }
    pub(super) async fn list_runs_impl(&self, workflow_id: Option<Uuid>) -> Result<Vec<Run>> {
        let runs = match workflow_id {
            Some(wf_id) => sqlx::query_as::<_, Run>("SELECT * FROM runs WHERE workflow_id = $1 ORDER BY created_at DESC").bind(wf_id).fetch_all(&self.pool).await?,
            None => sqlx::query_as::<_, Run>("SELECT * FROM runs ORDER BY created_at DESC").fetch_all(&self.pool).await?,
        };
        Ok(runs)
    }
